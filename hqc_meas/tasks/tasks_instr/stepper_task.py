...
from lantz_core.unit import UNIT_SUPPORT


def check_channel(driver, channel):
    if driver and channel not in driver.anm150.available:
...
class SetSteppingParametersTask(HackCheckInstrTask):
    ...
    def check(self, *args, **kwargs):
        ...
        """
        run_time = self.root_task.run_time
        test, traceback = super(SetSteppingParametersTask,
...
class SteppingTask(HackCheckInstrTask):
    ...
    #: Number of steps
    steps = Int(1).tag(pref=True)

    driver_list = ['ANC300']
    loopable = False
    parallel = set_default({'activated': True, 'pool': 'instr'})
    task_database_entries = set_default({'steps': 1})
    ...
    def perform(self):
        ...
        if not self.driver:
            self.start_driver()
        if self.driver.owner != self.task_name or not self.driver.connected:
            self.driver.anm150[self.channel].mode = 'Step'
            self.driver.owner = self.task_name

        channel = self.driver.anm150[self.channel]
        channel.step(self.direction, self.steps)
        time = self.steps/channel.frequency
        if UNIT_SUPPORT:
            time = time.m
        sleep(time)
